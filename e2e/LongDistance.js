...
describe('LongDistance test', function () {
	...
	var future = new Date(now.getTime() + msInDay * 4);//4 дн€ вперЄд
	var farFuture = new Date(now.getTime() + msInDay * 8);//8 дн€ вперЄд
	...
	it('go to client', function () {
		//----------------------------------закончили дела в админке, идЄм в клиента
	...
	it('confirming', function () {
		...
		TryToClickId('edit-status', function () {
			...
			trucks.count().then(function (c) {
				recurs = function (number, count, a, timeZone) { //передаЄм текущий номер, общее количество и массив строк и проход
					if (number < count) {  //≈сли ещЄ не дошли до конца
			...
			//---------------------------------добавл€ем packing и additional services
