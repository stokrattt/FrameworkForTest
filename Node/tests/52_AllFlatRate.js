...
condition.nowWeDoing = 'возвращаемся в диспач, проверяем что баланс 0, проверяем пейролл';
    ...
    LF.RememberAndValidatePayroll_In_EditRequest(V.boardNumbersPickup);
    SF.sleep (2);
...
condition.nowWeDoing = 'сейчас идём в пейролл и проверяем работy';
    ...
    driver.wait(driver.executeScript(JSstep.Payroll_GetForemanTotalForRequest(V.FRId)).then(function (text) {
        ...
        VD.IWant(VD.VToEqual, V.payrollNumbersDelivery.Foreman.Total, V.boardNumbersDelivery.Payroll.foremanForCommission.total, 'не совпали цифры Pickup в Payroll foreman\n' +
            'id=' + V.FRId);
    }), config.timeout);
    SF.sleep(1);
    MF.Payroll_ClickAllDepartment();
...
condition.nowWeDoing = 'проверяем цифры менеджера';
    ...
    SF.sleep(1);
    VD.IWant(VD.VToEqual, V.payrollNumbersPickup.Sale.Total, V.boardNumbersPickup.Payroll.managerForCommission.total, 'не совпали цифры в Payroll manager\n' +
        'id=' + V.FRId);
    SF.sleep(2);
