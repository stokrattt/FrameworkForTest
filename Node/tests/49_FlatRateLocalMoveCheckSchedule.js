...
condition.nowWeDoing = 'выбираем цифры helper';
    ...
    if (V.current1 == V.Dates1) {
        ...
        SF.sleep(5);
        driver.wait(driver.executeScript("return $('div.line1:contains("+V.request.Id+")').length").then (function (checkSchedule) {
            VD.INeed(VD.ToEqual, checkSchedule, 1, 'работа не нашлась в schedule по move date, реквест номер '+V.request.Id+'');
        }),config.timeout);
    } else {
        SF.click(By.xpath('//i[@ng-click="vm.nextMonth()"]'));
        SF.sleep(5);
        ...
        SF.sleep(5);
        driver.wait(driver.executeScript("return $('div.line1:contains("+V.request.Id+")').length").then (function (checkSchedule) {
            VD.INeed(VD.ToEqual, checkSchedule, 1, '(проверка два) работа не нашлась в schedule по move date, реквест номер '+V.request.Id+'');
        }),config.timeout);
    }
    SF.sleep(2);
